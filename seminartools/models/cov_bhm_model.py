...
class FixedMatrixCovariance(pm.gp.cov.Covariance):
    ...
    def diag(self, X):
        return self.omega.loc[X, X].diag()

    def full(self, X, Xs=None):
        return self.omega.loc[X, X].to_numpy()
        # if Xs is None:
        #     return self.omega
        # else:
        #     return self.omega

class CovBHMModel(BaseModel):
    def __init__(
        self,
        lags: int = 1,
    ...
    def fit(self, data: pd.DataFrame):
        ...
        data[self.target_column] = (
            data[self.target_column] - self.target_mean
        ) / self.target_std
        ...
        try:
            ...
            cov_matrix = cov_matrix.fillna(
                np.nanmean(
                    cov_matrix.values[np.triu_indices(len(self.countries), k=1)]
                )
            )
            cov_matrix = cov_matrix + 2 * np.eye(len(self.countries))  # Ensures matrix is PSD
            # divide by 3 to keep diagonal elements 1
            cov_matrix = cov_matrix / 3
        except Exception as e:
            print(e)
            print(data[self.country_column].unique())
            print(self.countries)
            raise 
            
        cov_matrix = FixedMatrixCovariance(omega=cov_matrix)
        ...
        with pm.Model(
            ...
            for regression_col in tqdm(
                ...
                regression_coefficients.append(
                    latent.prior(
                        f"regression_coefficients_{regression_col}", X=self.countries
                    )
                )
        ...
        self.country_intercepts = np.concatenate(
            [self.country_intercepts, mean_intercepts], axis=1
        )
        self.regression_coefficients = np.concatenate(
            [self.regression_coefficients, mean_coefficients], axis=1
        )
    ...
    def predict(self, data: pd.DataFrame, debug: bool = False):
        ...
        data["country_idx"] = (
            data[self.country_column]
            .map({country: idx for idx, country in enumerate(self.countries)})
            .fillna(len(self.countries))
        )  # `len(self.countries)` points to the new "mean country" coefficients
        data["country_idx"] = data["country_idx"].astype(int)

        # Prepare regression features from data
        regression_data = data[
            self.regression_columns
        ].values  # Shape: [rows, regression_cols]
        ...
        predictions = (
            ...
            * self.regression_coefficients[:, country_indices, :]
        ...
        if self.pointwise_aggregation_method == "mean":
            ...
            aggregated_predictions = np.apply_along_axis(
                getKDE, arr=predictions, axis=0
            )

        else:
            raise ValueError(
                f"Unsupported aggregation method: {self.pointwise_aggregation_method}"
            )
        ...
        if debug:
            predictions_df = pd.concat(
                [
                    predictions_df,
                    pd.DataFrame(
                        self.regression_coefficients[:, country_indices, :].mean(
                            axis=0
                        ),
                        columns=[
                            f"{col}_coefficient" for col in self.regression_columns
                        ],
                    ),
                ],
                axis=1,
        ...
        # Denormalize
        if (
            self.pointwise_aggregation_method == "mean"
            or self.pointwise_aggregation_method == "median"
        ):
            predictions_df["inflation"] = (
                predictions_df["inflation"] * self.target_std + self.target_mean
            )
            ...
            predictions_df.inflation = predictions_df.inflation.apply(
                denormalize_density
            )
